...
* **Zero-Trust Permissions (No `<all_urls>`):** As a privacy-first DLP extension, asking to read all websites is a fatal product flaw. We explicitly forbid the `<all_urls>` permission. We statically declare default AI domains (e.g., ChatGPT, Claude) in `manifest.json` and use the `scripting` permission (`chrome.scripting.registerContentScripts`) to dynamically inject the pipeline for user-configured custom domains.
