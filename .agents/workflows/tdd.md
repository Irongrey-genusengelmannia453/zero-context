...
When exploring the codebase, read `ARCHITECTURE.md` and `ROADMAP.md` so test names and interface vocabulary match the project's domain language, and respect architectural constraints in the area you're touching.

## What a good test is

Tests verify behavior through public interfaces, not implementation details. Code can change entirely; tests shouldn't. A good test reads like a specification — "lexer ignores standard JSON keys but extracts string values" tells you exactly what capability exists — and survives refactors because it doesn't care about internal structure.
